...
public class JavaTextDocumentSnippetRegistryTest {
	...
	public void jaxrsSnippets() {
		Optional<Snippet> jaxrsSnippet = findByPrefix("jrc", registry);
		Assert.assertTrue("Tests has jrc java snippet", jaxrsSnippet.isPresent());

		ISnippetContext<?> context = jaxrsSnippet.get().getContext();
		Assert.assertNotNull("jrc snippet has context", context);
		Assert.assertTrue("jrc snippet context is Java context", context instanceof SnippetContextForJava);
